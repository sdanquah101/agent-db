...
- **logs every attempt verbatim** to `runs/<id>/workflows/p1/llm_calls.jsonl` (§13). The
  ...
  and the request's sha256. `rebuild_requests` reassembles every request exactly (tested).
...
- `RecordedClient(transcript)` replays a logged run turn by turn and refuses a request
  whose digest differs from the recorded one. `python -m tools.runner --workflow p1 --run
  <id> --replay <llm_calls.jsonl>` re-runs a cell from its log.
...
- **Windows.** Fits, screening, Fisher, profiles, the sampler, residuals, assays and record
  inspection work on the calibration window `[0, 0.75 T]`. `validate` alone reads the
  frozen hold-out `[0.75 T, T]`, the evaluator's own (tested equal). QC and the mass
  balance read the whole record, as P0's do.
...
- **Workspace actions.** `set_sensor_status`, `record_evidence` and `conclude` fill the task
  ...
  - a posterior interval with no converged sampler call;
  ...
  unsupported-claim check, and it stays a measurement.
...
## 6. Open points (in the PR for the coordinator)
