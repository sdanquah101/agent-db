...
def rhs(
    ...
    for i, tr in enumerate(model.transfers):
        liquid = sp.S_co2 if tr.free_co2 else yr[tr.liquid]
        rho_T = pc.k_La * (liquid - tr.cod_per_kmol * tr.K_H * p[i])
        dy[tr.liquid] -= rho_T
        dy[tr.gas] = -yr[tr.gas] * q_raw / plant.V_gas + rho_T * ratio
    return dy
    ...
    *,
    ...
    for seg in segments:
        if lookup is None:
            ...
            def fun(t: float, yy: np.ndarray, _c: np.ndarray = conc, _q: float = q) -> np.ndarray:
                return rhs(t, yy, _c, _q, model)

        else:

            def fun(t: float, yy: np.ndarray) -> np.ndarray:
                c, q = lookup(t)
                return rhs(t, yy, c, q, model)
    ...
    return _assemble(t_out, y_out, model, success, message, stats)
