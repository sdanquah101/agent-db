"""Run the distinguishability analysis over a store's Level 0-5 cells.
    ...
    python -m distinguish --part 0/3                 # every third pair of Levels 0-5
    python -m distinguish --scenario S0-01           # one scenario's pairs
...
never read it) and, with ``--table``, the per-cell table. Every class is built on the
truth simulation (method version 4), so the pairs are independent and can run in any
order. A pair whose every run already has an ``admissible.json`` of the current method
version is skipped; ``--plant`` restricts to one plant.
"""
    ...
    for (_sid, _plant), members in _pairs(truth_store, levels):
        for r in sorted(members, key=lambda m: m["tier"]):
            ...
            row = {
                ...
                "simulations_pair": doc["simulations_pair"], "wall_s": doc["wall_s"],
                "pair_wall_s": doc["pair_wall_s"],
            }  # fmt: skip
            for lab in LABELS:
                c = doc["classes"].get(lab, {})
                row[f"delta_{lab}"] = c.get("delta")
                row[f"failed_{lab}"] = c.get("failed_candidates")
            rows.append(row)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--cache-dir", type=Path, default=None, help="keep the simulations")
    ...
    for n, ((sid, plant), members) in enumerate(_pairs(truth_store, range(0, 6))):
        if n % k != i or (args.scenario and sid not in args.scenario):
            continue
        if args.plant and plant != args.plant:
            continue
        ...
        docs = analyse_pair(runs, cache_dir=args.cache_dir, log=lambda m: print(m, flush=True))
