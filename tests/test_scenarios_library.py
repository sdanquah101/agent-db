...
#: two, Level 6 three, Level 7 two, Level 8 two. Nineteen in total, which is the number
#: §9.3's milestone row and gate G1 both quote.
LADDER_ROWS_PER_LEVEL = {0: 1, 1: 1, 2: 3, 3: 3, 4: 2, 5: 2, 6: 3, 7: 2, 8: 2}
LADDER_TOTAL = 19
...
def test_the_library_is_the_nineteen_row_ladder(library):
...
def test_structural_rows_abstain_and_ask_for_a_structural_review(library):
    ...
    for scenario in library.values():
        if TruthLabel.STRUCTURAL not in scenario.truth_label:
            continue
        assert scenario.correct_conclusion.recommend_structural_review, scenario.id
...
def test_the_ammonia_rows_are_the_three_the_proposal_names(library):
    ...
    assert len(AMMONIA_SCENARIOS) == 3
...
def test_the_matrix_has_the_shape_of_section_7():
    ...
    assert len(plant_a) == (10 - 1) * 1 + 3 * 3 == 18  # S5-01 is in both sets, counted once
    assert len(cells) == 96 + 18 == 114
...
def test_tiers_of_one_cell_share_one_truth_integration():
    ...
    assert len(keys) == 44, len(keys)  # 16 x 2 factorial + 12 distinct Plant A scenarios
    for key in keys:
